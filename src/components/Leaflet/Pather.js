...
export class Pather extends FeatureGroup {
  ...
  simplifyLine(latLngs) {
    const points = latLngs.map(({ lat, lng }) => ({ y: lat, x: lng }));
    const simplifiedPoints = rdpSimplification(
      points,
      this.options.simplifyThreshold
    );
  ...
  onRemove(map) {
    ...
    const tileLayer = map.getContainer().querySelector(".leaflet-tile-pane");
  ...
  mouseDown(event) {
    ...
    if ("button" in event.originalEvent && event.originalEvent.button === 1) {
      ...
      const eventClone = new MouseEvent(
    ...
    const lineIterator = this.createPathTemp(
    ...
    const mouseUp = () => {
      ...
      this.map.off("touchmove", mouseMove);
    ...
    this.map.on("touchmove", mouseMove);
    this.map.on("mouseup", mouseUp);
    this.map.on("touchend", mouseUp);
  }
}

function rdpSimplification(l, eps) {
